...
import IconImagesUrl from './_data/icon-images-url.json';
      ...
      <div>
        <div className='w-full max-w-xl'>
          ...
          <p className='text-primary/80 text-lg mt-5'>
            ...
            <span className='text-white'>a flexible toolkit</span> that makes every task feel like a breeze.
          </p>
        </div>
      </div>
          ...
          ImgComponent={() => (
            <div className='relative mb-4 flex h-full w-full items-center justify-center overflow-hidden [mask:linear-gradient(black_70%,transparent)]'>
              ...
              <div className='absolute bottom-0 right-12 sm:right-44 md:right-16 lg:right-40 xl:right-0'>
                <Image alt='hand' src={'/assets/images/toolkit/hand.png'} width={169} height={149} />
              </div>
            </div>
          )}
        />
          ...
          ImgComponent={() => (
            <div className='mb-4 h-full w-full overflow-hidden [mask:linear-gradient(transparent,black,transparent)]'>
              <ul role='list' className='relative space-y-4 pl-2 pr-4 pt-2'>
                ...
                <Alert
                  time='2h ago'
                  contents={['💳 Monica is requesting a new card', 'Card type: Virtual', 'Spend limit: $500 monthly']}
                  buttons={['Approve', 'Decline', 'Edit']}
                />
                <Alert time='52m ago' contents={['📈 Revenue increase alert', '27% increase in the last 7 days']} />
                <Alert time='Just now' contents={['🚨 Large expense alert', 'Amount: $2,487.92']} />
              </ul>
            </div>
          )}
        />
          ...
          ImgComponent={() => (
            <div className='[mask:radial-gradient(60%_60%_at_50%,rgba(0,0,0,0.9)_35%,rgba(0,0,0,0)_100%)]'>
              <div className='flex justify-between pr-10'>
                {IconImagesUrl?.map((icons, index) => (
                  <div key={index} className={`-ml-4 flex flex-col gap-8 sm:ml-8 ${index % 2 == 1 && 'pt-8'}`}>
                    {icons?.map((imageUrl, idx) => (
                      <span
                        ...
                        key={`toolkit-${idx}`}>
                        <Image width={50} height={50} alt='icon' src={imageUrl} />
                      </span>
                    ))}
                  </div>
                ))}
            ...
            <div className='relative flex h-full w-full items-center justify-center overflow-hidden'>
              ...
              <div className='absolute -left-16 bottom-10 h-[243px] w-[891px] sm:-left-12 lg:left-8'>
                <Image
                  width={800}
                  height={100}
                  src={'/assets/images/toolkit/keys.png'}
                  alt='keyboard'
                ...
                <Image
                  width={800}
                  height={100}
                  src={'/assets/images/toolkit/keyboard.png'}
                  alt='keyboard'
